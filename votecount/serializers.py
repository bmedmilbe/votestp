...
from .tasks import aggregate_votetable_results_task
...
class PollingStationSerializer(serializers.ModelSerializer):
    ...
    circunscricao_code = serializers.CharField(
        source="circunscricao.code", read_only=True
    )
    circunscricao_name = serializers.CharField(
        source="circunscricao.name", read_only=True
    )
...
class VoteEntrySerializer(serializers.ModelSerializer):
    ...
    party_abbreviation = serializers.CharField(
        source="party.abbreviation", read_only=True
    )
...
class VoteTableSerializer(serializers.ModelSerializer):
    ...
    polling_station_name = serializers.CharField(
        source="polling_station.name", read_only=True
    )
    district_name = serializers.CharField(
        source="circunscricao.district.name", read_only=True
    )
...
class VoteTableCreateSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        # 1. Update the top-level VoteTable fields natively
        instance.invalid_votes = validated_data.get(
            "invalid_votes", instance.invalid_votes
        )
        instance.blank_votes = validated_data.get("blank_votes", instance.blank_votes)
        instance.save()

        # 2. Extract incoming party data from the payload
        parties_data = validated_data.pop("vote_entries", [])

        # 3. Sort by party ID to guarantee consistent, sequential row locking order
        parties_data.sort(
            key=lambda x: x["party"].id if hasattr(x["party"], "id") else x["party"]
        )

        # 4. Instantiate objects in memory instead of hitting the database repeatedly
        vote_entry_objects = []
        for party_data in parties_data:
            party = party_data.pop("party")

            vote_entry_objects.append(
                VoteEntry(
                    vote_table=instance,
                    party=party,
                    votes_count=party_data.get(
                        "votes_count", 0
                    ),  # Adjust matching field names
                )
            )

        # 5. Execute a single bulk database transaction (SQL ON CONFLICT DO UPDATE)
        if vote_entry_objects:
            VoteEntry.objects.bulk_create(
                vote_entry_objects,
                update_conflicts=True,
                unique_fields=[
                    "vote_table",
                    "party",
                ],  # Indexes defining unique constraint
                update_fields=["votes_count"],  # Columns to rewrite upon conflict
            )

        # 6. Hand off a single aggregation workflow to Celery once the outer transaction safely commits
        transaction.on_commit(
            lambda: aggregate_votetable_results_task.delay(instance.id)
        )
...
class NestedDistrictSerializer(serializers.ModelSerializer):
    ...
    def get_results(self, obj):
        results = ResultPerDistrictPerParty.objects.filter(district=obj).select_related(
            "party"
        )
        return ResultPerPartySerializer(results, many=True).data
    ...
    def get_total_voters(self, obj):
        return (
            VoteTable.objects.filter(circunscricao__district=obj).aggregate(
...
class NestedCountrySerializer(serializers.ModelSerializer):
    ...
    def get_results(self, obj):
        results = ResultPerCountryPerParty.objects.filter(country=obj).select_related(
            "party"
        )
        return ResultPerPartySerializer(results, many=True).data

    def get_deputies(self, obj):
        deputies = ResultPerCountryPerParty.objects.filter(country=obj).select_related(
            "party"
        )
        return ResultPerPartySerializer(deputies, many=True).data
    ...
    def get_total_voters(self, obj):
        return (
            VoteTable.objects.filter(circunscricao__district__country=obj).aggregate(
                total=Sum("total_voters")
            )["total"]
            or 0
        )
