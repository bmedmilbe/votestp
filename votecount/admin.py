...
class CountryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Country Information", {"fields": ("name", "code", "total_deputies")}),
    )
...
class CircunscricaoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Circunscrição Information", {"fields": ("code", "name", "district")}),
    )
...
class PollingStationAdmin(admin.ModelAdmin):
    ...
    ordering = ["name"]
    fieldsets = (
        ("Polling Station Information", {"fields": ("name", "circunscricao")}),
    )
...
class VoteTableAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "Timestamps",
            {"fields": ("recorded_at", "updated_at"), "classes": ("collapse",)},
        ),
    )
...
class VoteEntryAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Vote Entry Information", {"fields": ("vote_table", "party", "votes_count")}),
        (
            "Timestamps",
            {"fields": ("recorded_at", "updated_at"), "classes": ("collapse",)},
        ),
...
class ResultPerCountryPerPartyAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Result Information", {"fields": ("country", "party", "result", "deputies")}),
    )
...
class ResultPerDistrictPerPartyAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Result Information", {"fields": ("district", "party", "result", "deputies")}),
    )
...
class ResultPerCircunscricaoPerPartyAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Result Information", {"fields": ("circunscricao", "party", "result")}),
    )
...
class VoteSummaryAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        total_voters = (
            VoteTable.objects.aggregate(total=Sum("total_voters"))["total"] or 0
        )
        total_votes = (
            VoteEntry.objects.aggregate(total=Sum("votes_count"))["total"] or 0
        )

        # Results by party
        results_by_party = (
            VoteEntry.objects.values(
