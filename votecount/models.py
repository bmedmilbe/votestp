...
class CountryManager(models.Manager):
    ...
    def with_vote_table_counts(self):
        ...
        return self.get_queryset().annotate(
            vote_tables_count=Count('districts__circunscricoes__vote_tables', distinct=True)
        )
    ...
    def with_full_stats(self):
        ...
        return self.get_queryset().annotate(
            ...
            polling_stations_count=Count('districts__circunscricoes__polling_stations', distinct=True),
            vote_tables_count=Count('districts__circunscricoes__vote_tables', distinct=True),
...
class DistrictManager(models.Manager):
    ...
    def with_full_stats(self):
        ...
        return self.get_queryset().annotate(
            circunscricoes_count=Count('circunscricoes', distinct=True),
            polling_stations_count=Count('circunscricoes__polling_stations', distinct=True),
            vote_tables_count=Count('circunscricoes__vote_tables', distinct=True),
    ...
    def with_all_relations(self):
        ...
        return self.get_queryset().select_related('country').prefetch_related(
            'circunscricoes',
...
class PollingStationManager(models.Manager):
    ...
    def get_queryset(self):
        return super().get_queryset().select_related('circunscricao', 'circunscricao__district', 'circunscricao__district__country')
...
class VoteEntryManager(models.Manager):
    ...
    def by_district(self, district_id):
        ...
        return self.get_queryset().filter(vote_table__circunscricao__district_id=district_id)
    ...
    def with_party_totals(self):
        ...
        return self.get_queryset().values(
            'party_id', 'party__name', 'party__abbreviation', 'party__color'
        ).annotate(
