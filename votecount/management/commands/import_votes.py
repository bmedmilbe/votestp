...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            'json_file',
            type=str,
            help='Path to the JSON file containing voting data'
        )
        parser.add_argument(
            '--country',
            type=str,
        ...
        try:
            with transaction.atomic():
                ...
                if clear_existing:
                    ...
                    self.stdout.write(self.style.SUCCESS(f"✅ Cleared existing data for {country_name}"))
                ...
                for district_data in data:
                    ...
                    if not district_info:
                        self.stdout.write(self.style.WARNING("⚠️ Skipping empty district data"))
                        continue
                ...
                # Save import record
                import_record = OriginalDataImport.objects.create(
                    json_data=data,
                    imported_by=user_email,
                    notes=f"Imported from {json_file_path} at {timezone.now()}"
                )
                ...
                self.stdout.write(self.style.SUCCESS(
                    ...
                    f"  Import record ID: {import_record.id}"
                ))
    ...
    def process_district(self, district_info, country):
        ...
        district, created = District.objects.get_or_create(
            name=district_name,
            sigla=district_sigla,
            country=country
        )

        if created:
            self.stdout.write(f"  📍 Creating district: {district_name} ({district_sigla})")
        ...
        for localidade in localidades:
            try:
                ...
                if not circunscricao_code:
                    self.stdout.write(self.style.WARNING("    ⚠️ Missing Circunscrição in localidade"))
                    continue
                ...
                self.stdout.write(
                    self.style.WARNING(f"    ⚠️ Error processing {localidade.get('Circunscrição', 'Unknown')}: {e!s}")
                )
                raise
    ...
    def process_vote_table(self, localidade, district, circunscricao_code):
        ...
        polling_station, created = PollingStation.objects.get_or_create(
            ...
            defaults={'circunscricao': circunscricao}
        )
